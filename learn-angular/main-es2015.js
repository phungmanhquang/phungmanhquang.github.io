...
let ChildCompanent = class ChildCompanent {
    constructor() {
...
let NgFormComponent = class NgFormComponent {
    constructor(ngFormService) {
...
NgFormComponent.ctorParameters = () => [
...
let ngFormService = class ngFormService {
    constructor(http) {
        this.http = http;
    }
    sendPost(value) {
        const url = "http://localhost:3000/postForm";
        const header = new _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpHeaders"]({ 'Content-Type': 'application/json', });
        const body = JSON.stringify(value);
        return this.http.post(url, body, { headers: header })
            .toPromise();
    }
};
ngFormService.ctorParameters = () => [
...
let ParentCompanent = class ParentCompanent {
    constructor() {
...
let roudNum = class roudNum {
    transform(value, isUpper, toAdd) {
        if (isUpper) {
            // ceil lam tron len
...
let sigupComponent = class sigupComponent {
    constructor(fb) {
        this.fb = fb;
    }
    ngOnInit() {
...
let StructComponent = class StructComponent {
    ...
    Delete(id) {
        const index = this.arrWords.findIndex(show => show.id === id);
        this.arrWords.splice(index, 1);
    }
    getLocTrangThai(memorized) {
        const TatCa = this.TrangThai === 'Xem_TatCa';
        const DaNho = this.TrangThai === 'Xem_DaNho' && memorized;
        const ChuaNho = this.TrangThai === 'Xem_ChuaNho' && !memorized;
...
let UserFromComponent = class UserFromComponent {
    constructor() {
        this.name = '';
